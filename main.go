...
func main() {
	...
	fmt.Println("Price one apple", priceOneApple)
	fmt.Println("Price one pear", priceOnePear)
	fmt.Println("We have money", haveMoney)
	fmt.Println("")
	costApples = countApples * priceOneApple
	costPears = countPears * priceOnePear
	sumMoney = costApples + costPears
	fmt.Println("How much money do you have to spend to buy 9 apples and 8 pears?", sumMoney)
	fmt.Println("")
	buyPears := float64(haveMoney) / float64(priceOnePear)
	int, frac := math.Modf(buyPears)
	_ = frac
	fmt.Println("How many pears can we buy?", int)
	fmt.Println("")
	buyApples := float64(haveMoney) / float64(priceOneApple)
	i, f := math.Modf(buyApples)
	_ = f
	fmt.Println("How many apples can we buy?", i)
	fmt.Println("")
